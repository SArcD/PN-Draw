...
# Función para ruido Perlin simple
def perlin_noise(x, y, scale=10):
    return (np.sin(x / scale) + np.cos(y / scale)) * 0.5 + 0.5

# Función para generar fractales de bifurcación
def generate_fractal_branches(x, y, length, angle, depth, scale=10, opacity=0.7):
    ...
    # Agregar la rama actual con opacidad basada en ruido Perlin
    noise_value = perlin_noise(x, y, scale)
    alpha = max(0.1, min(opacity * noise_value, 1.0))  # Asegurar opacidad entre 0.1 y 1.0
    branches.append(((x, y, x_end, y_end), alpha))

    # Recursión para bifurcaciones
    new_length = length * 0.7
    branches += generate_fractal_branches(x_end, y_end, new_length, angle - 20, depth - 1, scale, opacity)
    branches += generate_fractal_branches(x_end, y_end, new_length, angle + 20, depth - 1, scale, opacity)

    return branches

# Función principal para dibujar fractales en espiral
def draw_fractals_on_spiral(center, a, b, num_turns, num_fractals, depth, scale):
    theta = 0
    branches = []
    for i in range(num_fractals):
        ...
        # Generar ramas de fractal desde el punto de la espiral
        branches += generate_fractal_branches(x_spiral, y_spiral, length=50, angle=theta * 180 / np.pi, depth=depth, scale=scale)
        theta += (2 * np.pi * num_turns) / num_fractals
    return branches

# Streamlit UI
st.title("Visualización de Fractales en Espiral con Perlin y Bordes Difusos")
st.sidebar.markdown("### Parámetros del Fractal y la Espiral")

# Parámetros ajustables con deslizadores
...
scale = st.sidebar.slider("Escala de Ruido Perlin", 5, 50, 20)
opacity = st.sidebar.slider("Opacidad Máxima", 0.1, 1.0, 0.7)

# Generar fractales
center = (0, 0)
branches = draw_fractals_on_spiral(center, a, b, num_turns, num_fractals, depth, scale)

# Visualización con Plotly
fig = go.Figure()

# Dibujar elipses de referencia
...
fig.update_layout(
    ...
    title="Fractales en Espiral con Bordes Difusos y Ruido Perlin",
)
